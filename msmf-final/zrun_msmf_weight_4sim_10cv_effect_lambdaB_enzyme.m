...
for drugOrTarget = drugOrTargetArray,
    ...
    learn_rateA = 0.0000005;
    learn_rateB = 0.0000005;
    lambda_list = [1];
    lambdaA_list = 256;
    lambdaB_list = [1,2,4,8,16,32,64,128,256,512,1024];
    
    file = ['result/result_1times10fold_msmf_weight_4sim_effect_lambdaB_', dataset, '.txt'];
    fid = fopen(file,'a');
        ...
        for iter = iter_list,
            for learn_rate = learn_rate_list,
                for lambda = lambda_list,
                    for lambdaA = lambdaA_list,
                        for lambdaB = lambdaB_list,
                            %------------------------------
                            % x-time-10-fold-cv
                            for time=1:1,
                                t0 = clock;
